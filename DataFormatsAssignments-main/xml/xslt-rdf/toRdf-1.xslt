...
&lt;studio/<xsl:value-of select="translate(substring-after(@id, 'studio_'), '-', '')"/>&gt; a ex:Studio ;
...
&lt;cinema/<xsl:value-of select="translate(substring-after(@id, 'cinema_'), '-', '')"/>&gt; a ex:Cinema ;
    foaf:name "<xsl:value-of select="md:Name"/>"<xsl:if test="md:Name/@xml:lang">@<xsl:value-of select="md:Name/@xml:lang"/></xsl:if> ;
...
<xsl:text>
    ...
    <xsl:template match="md:Person">
        <xsl:variable name="personId" select="translate(substring-after(@id, 'person_'), '-', '')"/>
...
&lt;movie/<xsl:value-of select="translate(substring-after(@id, 'movie_'), '-', '')"/>&gt; a ex:Movie ;
    ...
    ex:directedBy &lt;person/<xsl:value-of select="translate(substring-after(md:DirectedBy/@ref, 'person_'), '-', '')"/>&gt; ;
    ex:producedBy &lt;studio/<xsl:value-of select="translate(substring-after(md:ProducedBy/@ref, 'studio_'), '-', '')"/>&gt; .
<xsl:text>
</xsl:text>
    </xsl:template>
    
    <xsl:template match="md:Screening">
&lt;screening/<xsl:value-of select="translate(substring-after(@id, 'screening_'), '-', '')"/>&gt; a ex:Screening ;
    ex:shows &lt;movie/<xsl:value-of select="translate(substring-after(md:Shows/@ref, 'movie_'), '-', '')"/>&gt; ;
    ex:takesPlaceAt &lt;cinema/<xsl:value-of select="translate(substring-after(md:TakesPlaceAt/@ref, 'cinema_'), '-', '')"/>&gt; ;
...
&lt;person/<xsl:value-of select="translate(substring-after(../@id, 'person_'), '-', '')"/>&gt; ex:actedIn &lt;movie/<xsl:value-of select="translate(substring-after(@ref, 'movie_'), '-', '')"/>&gt; .
<xsl:text>
</xsl:text>
    </xsl:template>
